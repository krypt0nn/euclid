...
fn main() {
    ...
    type GenericModel = GenericWordEmbeddingsModel::<1024, 2, 8, f64>;
    ...
    for i in 0..300 {
        ...
        if i % 20 == 0 && i != 0 {
            ...
            if i % 100 == 0 && i != 0 {
                let cosine     = model.encode(cosine);
                let similarity = model.encode(similarity);
                let noteworthy = model.encode(noteworthy);
