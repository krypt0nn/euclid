...
/// Generic recurrent generator model using Long Short-Term Memory cells.
pub struct GenericModel<const INPUT_SIZE: usize, const OUTPUT_SIZE: usize, F: Float> {
    pub lstm_layer: LSTMLayer<INPUT_SIZE, F>,
    pub output_layer: Layer<INPUT_SIZE, OUTPUT_SIZE, F>
}

impl<const INPUT_SIZE: usize, const OUTPUT_SIZE: usize, F: Float> GenericModel<INPUT_SIZE, OUTPUT_SIZE, F> {
    ...
    pub fn train(
        ...
        sequence: &[F],
        step: usize,
        policy: &mut BackpropagationSnapshot<{ GenericModel::<INPUT_SIZE, OUTPUT_SIZE, F>::PARAMS }, F>,
        device: &mut impl Device
    )
    where
        [(); { GenericModel::<INPUT_SIZE, OUTPUT_SIZE, F>::PARAMS }]: Sized,
